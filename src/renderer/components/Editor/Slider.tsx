 ...
 * Slider Component
 *
 * Visual representation of a linear fader control.
...
export function Slider({ value, label, controlId, onValueChange, selected = false, onToggleSelection }: SliderProps): React.ReactElement {
  ...
  // Calculate fill percentage (0-127 maps to 0-100%)
  const fillPercent = (value / 127) * 100;
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    const handleMouseMove = (moveEvent: MouseEvent) => {
      ...
      if (hasDragged) {
        const sensitivity = 127 / rect.height;
        const newValue = Math.max(0, Math.min(127, Math.round(startValue + deltaY * sensitivity)));
  ...
  return (
    <>
      ...
      >
        ...
        <div
          ref={trackRef}
          className={`slider-track ${isDragging ? 'ring-2 ring-nk-accent' : ''}`}
        ...
        >
          ...
          {/* Fader cap */}
          <div
            className="absolute left-1/2 -translate-x-1/2 w-8 h-3 bg-gray-400 rounded shadow-md transition-all"
